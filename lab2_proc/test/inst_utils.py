...
#-------------------------------------------------------------------------
# Test the destination bypass path by varying how many nops are
# inserted between the instruction under test and reading memory
# with a lw and csrr instruction.

def gen_st_dest_dep_test( num_nops, inst, base, src, result ):
  return gen_st_template( 8, 8, num_nops, 'x1', 'x2', inst, 0, base, src, result )
...
def gen_st_value_test( inst, offset, base, src,  result ):
  return gen_st_template( 0, 0, 0, "x1", "x2", inst, offset, base, src, result )
